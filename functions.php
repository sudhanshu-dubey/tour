...
function load_custom_script() {
    ...
    wp_enqueue_script( 'google-jquery-ui', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.12/jquery-ui.min.js', array( 'jquery' ) );
...
function wpse64933_add_posttype_note()
{
    ...
    echo '<div id="custom_container"></div>';
}
...
function fetch_new_data(){
	...
	$mydata=$wpdb->get_results("SELECT * FROM wp_tour ORDER BY step",ARRAY_A );
	//print_r($mydata);
	//$mydata = $wpdb->get_row( "SELECT * FROM wp_tour", ARRAY_A );
	//echo json_encode(array('data_tour'=>$mydata));

	echo '<div id="json_fetched_data" style="display:none;">'.json_encode(array('data_tour'=>$mydata)).'</div>';

}
add_action('admin_init','fetch_new_data');